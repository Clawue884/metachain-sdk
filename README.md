...
TypeScript library for client-side interactions with metachain. metachain SDK provides common helper functionality as well as access to the underlying smart contract interfaces.

Below is an overview of the metachain SDK functionality. See the [tutorials](https://github.com/META-MetaChain/metachain-tutorials) for further examples of how to use these classes.

### Quickstart Recipes

- ##### Deposit Ether Into metachain

```ts
import { getL2Network, EthBridger } from '@metachain/sdk'

const l2Network = await getL2Network(
  l2ChainID /** <-- chain id of target metachain chain */
)
...
```ts
import { L1TransactionReceipt, L1ToL2MessageStatus } from '@metachain/sdk'
...
```ts
import { L2TransactionReceipt } from '@metachain/sdk'

const l2TxnReceipt = new L2TransactionReceipt(
  txnReceipt /** <-- ethers-js TransactionReceipt of an metachain tx */
)
...
metachain SDK can be used to bridge assets to/from the rollup chain. The following asset bridgers are currently available:
...
metachain SDK comes pre-configured for Mainnet and Goerli, and their metachain counterparts. However, the networks functionality can be used to register networks for custom metachain instances. Most of the classes in metachain SDK depend on network objects so this must be configured before using other metachain SDK functionality.

### Inbox tools

As part of normal operation the metachain sequencer will send messages into the rollup chain. However, if the sequencer is unavailable and not posting batches, the inbox tools can be used to force the inclusion of transactions into the rollup chain.
...
- **constants** - A list of useful metachain related constants

### Run Integration tests

1. First, make sure you have a Nitro test node running. Follow the instructions [here](https://docs.metachain.cash/node-running/how-tos/local-dev-node).
...
Defaults to `metachain Goerli`, for custom network use `--network` flag.

`metachain Goerli` expects env var `META_KEY` to be prefunded with at least 0.02 ETH, and env var `INFURA_KEY` to be set.
