...
const METAKey = process.env['META_KEY'] as string
...
export const instantiateBridge = (
  ...
  if (!l1PkParam && !ethKey) {
    throw new Error('need META_KEY var')
  }
  if (!l2PkParam && !METAKey) {
    throw new Error('need META_KEY var')
  }
  ...
  const l2Rpc = (() => {
    if (l2NetworkID === 42161)
      return process.env['META_ONE_RPC'] || 'https://META1.metachain.cash/rpc'
    if (l2NetworkID === 421613)
      return (
        process.env['GOERLI_ROLLUP_TESTNET_RPC'] ||
        'https://goerli-rollup.metachain.cash/rpc'
      )
    throw new Error(
  ...
  const METAProvider = new JsonRpcProvider(l2Rpc)
  ...
  const l2Signer = (() => {
    if (l2PkParam) {
      return new Wallet(l2PkParam, METAProvider)
    } else if (METAKey) {
      return new Wallet(METAKey, METAProvider)
    } else {
      throw new Error('impossible path')
    }
  })()
