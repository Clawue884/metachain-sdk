...
import { metachainProvider } from '../../src/lib/utils/METAProvider'
import { METABlock } from '../../src/lib/dataEntities/rpc'

describe('L2 blocks lookup for an L1 block', () => {
  const provider = new JsonRpcProvider('https://META1.metachain.cash/rpc')
  const METAProvider = new metachainProvider(provider)
  ...
  async function validateL2Blocks({
    ...
    const promises: Promise<METABlock>[] = []
    ...
    l2Blocks.forEach((l2Block, index) => {
      ...
      promises.push(METAProvider.getBlock(l2Block))
      // Search for previous or next block.
      promises.push(METAProvider.getBlock(l2Block + (isStartBlock ? -1 : 1)))
    })
    ...
    if (startBlock && blockBeforeStartBlock) {
      ...
      // Check if metachain start block is the first block for this L1 block.
      expect(
        startBlockCondition,
    ...
    if (endBlock && blockAfterEndBlock) {
      ...
      // Check if metachain end block is the last block for this L1 block.
      expect(
        endBlockCondition,
